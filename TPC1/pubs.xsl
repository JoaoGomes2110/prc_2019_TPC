...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/bibliography/*">
        ...
        ###  http://www.prc.di.uminho.pt/2019/pubs#<xsl:value-of select="@id"/>
        ...
        <xsl:for-each select="author">
            :<xsl:value-of select="$idpub"/> :hasAutor :<xsl:value-of select="@id"/> .
        </xsl:for-each>
        ...
        <xsl:for-each select="author-ref">
            :<xsl:value-of select="$idpub"/> :hasAutor :<xsl:value-of select="@authorid"/> .
        </xsl:for-each>
